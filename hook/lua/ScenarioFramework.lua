...
WaterVisionResetThread = function(self)
    ...
    while true do
        ...
        local units = SeabedRevealingUnits 
        ...
        for key, unit in units do
            if not unit.Dead then
                if unit.WVizEndTick and unit.WVizEndTick <= tick then
                    unit.WVizEndTick = nil 
                    if unit.NormalWVision then
                        --WARN('reseting wvision radius')
                        unit:SetIntelRadius('watervision', unit.NormalWVision)
                        unit.NormalWVision = nil 
                    else
                        --WARN('ABM:Unit is not dead and has expired watervision timestamp but no NormalWVision! Attempting to set to Blueprint value, then removing from table!')
